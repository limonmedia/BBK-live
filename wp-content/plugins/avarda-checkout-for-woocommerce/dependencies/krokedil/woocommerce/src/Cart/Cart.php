 ...
 */
namespace KrokedilAvardaDeps\Krokedil\WooCommerce\Cart;

use KrokedilAvardaDeps\Krokedil\WooCommerce\OrderData;
/**
 * Class to generate a cart object from the WooCommerce cart.
 */
class Cart extends OrderData
{
    ...
    public function set_line_shipping()
    {
        if ($this->cart->needs_shipping() && $this->cart->show_shipping()) {
            ...
            if (\class_exists('KrokedilAvardaDeps\\WC_Subscriptions_Cart') && \KrokedilAvardaDeps\WC_Subscriptions_Cart::all_cart_items_have_free_trial()) {
            ...
            foreach ($packages as $index => $package) {
                foreach ($package['contents'] as $cart_item_key => $cart_item) {
                    if (\class_exists('KrokedilAvardaDeps\\WC_Subscriptions_Product') && \KrokedilAvardaDeps\WC_Subscriptions_Product::get_trial_length($cart_item['data']) > 0) {
            ...
            foreach ($shipping_ids as $key => $shipping_id) {
                ...
                if (\class_exists('KrokedilAvardaDeps\\WC_Subscriptions_Cart') && \KrokedilAvardaDeps\WC_Subscriptions_Cart::cart_contains_subscription()) {
    ...
    public function set_line_coupons()
    {
        ...
        // WC Giftcards.
        if (\class_exists('KrokedilAvardaDeps\\WC_GC_Gift_Cards')) {
        ...
        // YITY Giftcards.
        if (\class_exists('KrokedilAvardaDeps\\YITH_YWGC_Gift_Card')) {
