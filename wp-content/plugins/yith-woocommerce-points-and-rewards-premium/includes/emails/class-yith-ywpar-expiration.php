<?php
/**
 * Implements Expiration email class
 *
...
if ( ! class_exists( 'YITH_YWPAR_Expiration' ) ) {
	...
	class YITH_YWPAR_Expiration extends WC_Email {
		...
		public function __construct() {
			...
			$this->title       = esc_html__( 'Expiring Points', 'yith-woocommerce-points-and-rewards' );
			$this->description = esc_html__( 'This email is sent to users, a few days before their points expire.', 'yith-woocommerce-points-and-rewards' );

			$this->heading = esc_html__( "Don't lose your points!", 'yith-woocommerce-points-and-rewards' );
			$this->subject = esc_html__( '[Expiring points]', 'yith-woocommerce-points-and-rewards' );
		...
		public function trigger( $args ) {
			...
			if ( ! $this->is_enabled() || ywpar_get_option( 'send_email_before_expiration_date' ) !== 'yes' ) {
				return;
			}
		...
		public function init_form_fields() {
			$this->form_fields = array(
				...
				'email_type'        => array(
					...
					'type'        => 'select',
					'description' => __( 'Choose the format of the email that will be sent.', 'yith-woocommerce-points-and-rewards' ),
					'default'     => 'html',
					'class'       => 'email_type',
					'options'     => array(
