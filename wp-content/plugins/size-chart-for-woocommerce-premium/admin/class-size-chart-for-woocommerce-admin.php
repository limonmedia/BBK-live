...
class SCFW_Size_Chart_For_Woocommerce_Admin {
	...
	public function scfw_enqueue_styles_scripts_callback( $hook_suffix ) {
		...
		if ( false !== strpos( $hook_suffix, '_page_size-chart' ) || false !== strpos( $hook_suffix, '_page_scfw' ) || 'size-chart' === $screen->post_type || 'product' === $screen->post_type ) {
			// Register styles.
			wp_register_style( $this->get_plugin_dash_name() . '-jquery-editable-style', plugin_dir_url( __FILE__ ) . 'css/jquery.edittable.css', array(), $this->get_plugin_version(), 'all' );
			wp_register_style( $this->get_plugin_dash_name() . '-font-awesome-style', plugin_dir_url( __FILE__ ) . 'css/font-awesome.min.css', array(), $this->get_plugin_version(), 'all' );
			wp_register_style( $this->get_plugin_dash_name() . '-select2-style', plugin_dir_url( __FILE__ ) . 'css/select2.css', array(), $this->get_plugin_version(), 'all' );
			wp_register_style( $this->get_plugin_dash_name(), plugin_dir_url( __FILE__ ) . 'css/size-chart-for-woocommerce-admin.css', array(), $this->get_plugin_version(), 'all' );
			wp_register_style( $this->get_plugin_dash_name() . '-main-style', plugin_dir_url( __FILE__ ) . 'css/style.css', array(), $this->get_plugin_version(), 'all' );

			if ( 'product' !== $screen->post_type ) {
				wp_register_style( $this->get_plugin_dash_name() . '-plugin-new-style', plugin_dir_url( __FILE__ ) . 'css/plugin-new-style.css', array(), $this->get_plugin_version(), 'all' );
			}
			wp_register_style( $this->get_plugin_dash_name() . '-plugin-setup-wizard', plugin_dir_url( __FILE__ ) . 'css/plugin-setup-wizard.css', array(), $this->get_plugin_version(), 'all' );
			if ( !( scfw_fs()->is__premium_only() && scfw_fs()->can_use_premium_code() ) ) {
				wp_register_style( $this->get_plugin_dash_name() . '-upgrade-dashboard-style', plugin_dir_url( __FILE__ ) . 'css/upgrade-dashboard.css', array(), $this->get_plugin_version(), 'all' );
			...
			// Register scripts.
			if ( 'product' !== $screen->post_type ) {
				wp_enqueue_script($this->get_plugin_dash_name() . '-help-scout-beacon-js', plugin_dir_url(__FILE__) . 'js/help-scout-beacon.js', array('jquery'), $this->get_plugin_version(), false);
			}
			wp_register_script( $this->get_plugin_dash_name() . '-jquery-editable-js', plugin_dir_url( __FILE__ ) . 'js/jquery.edittable.js', array( 'jquery' ), $this->get_plugin_version(), false );
			wp_register_script( $this->get_plugin_dash_name(), plugin_dir_url( __FILE__ ) . 'js/size-chart-for-woocommerce-admin.js', array(
				'jquery',
				'wp-color-picker',
				'selectWoo'
	        ...
	        if ( scfw_fs()->is__premium_only() & scfw_fs()->can_use_premium_code() ) {
	            $chart_table_style['custom-style'] = array(
	                'header_bg_color' => function_exists( 'scfw_size_chart_get_table_head_color' ) ? scfw_size_chart_get_table_head_color() : '#000',
	                'even_row_bg_color' => function_exists( 'scfw_size_chart_get_table_row_even_color' ) ? scfw_size_chart_get_table_row_even_color() : '#fff',
	                'odd_row_bg_color' => function_exists( 'scfw_size_chart_get_table_row_odd_color' ) ? scfw_size_chart_get_table_row_odd_color() : '#ebe9eb',
	                'text_color' => function_exists( 'scfw_size_chart_get_table_head_font_color' ) ? scfw_size_chart_get_table_head_font_color() : '#fff',
	                'even_text_color' => '#000',
	                'odd_text_color' => '#000',
	                'border_color' => '#d1d1d1',
	                'border_hb_style' => 'solid',
			...
			wp_localize_script(
				...
				$size_chart_localize_script_args
			);
			// Enqueue scripts.
			...
			wp_enqueue_script( $this->get_plugin_dash_name() . '-select2_js', plugin_dir_url(__FILE__) . 'js/select2.full.min.js?ver=4.0.3', array('jquery'), '4.0.3', false);
			//Premium popup
            wp_enqueue_script(
                'freemius',
			...
			if ( 'post.php' === $hook_suffix ) {
				...
				if ( $this->get_plugin_post_type_name() === $post->post_type && isset( $post_status ) && 'default' === $post_status ) {
					...
					wp_add_inline_script(
						$this->get_plugin_dash_name(),
						"window.onload = function() {jQuery('#title').prop('disabled', true);};"
					);
				}
			}
		}
	...
	public function scfw_size_chart_register_post_type_chart_callback() {
		$labels = array(
			...
			'add_new_item'       => __( 'Add New', 'size-chart-for-woocommerce' ),
	...
	public function scfw_size_chart_welcome_page_screen_and_menu_callback() {
		...
		add_submenu_page( 'dots_store',
			...
			array(
				$this,
				'scfw_size_chart_get_started_page'
			) );
		add_submenu_page( 'dots_store',
		...
		$settings = add_submenu_page( 'edit.php?post_type=size-chart',
	    ...
	    if ( $current_screen->post_type === 'size-chart' ) {
	        ...
	        <script type="text/javascript">
	            document.addEventListener('DOMContentLoaded', function() {
	                ...
	                if (inputElement) {
	                	inputElement.setAttribute('placeholder', '<?php echo esc_attr($placeholder); ?>');
	                }
	            });
	        </script>
	        <?php
	...
	public function scfw_size_chart_settings_page_callback() {
		...
		if ( isset( $size_chart_submit ) && isset( $get_page_name ) && 'size-chart-setting-page' === $get_page_name ) {
			...
			$nonce = filter_input( INPUT_POST, 'size_chart_gb_settings_nonce', FILTER_SANITIZE_FULL_SPECIAL_CHARS );
			if ( ! $nonce || ! wp_verify_nonce( $nonce, 'size_chart_gb_settings_action' ) ) {
			    wp_die( esc_html__( 'Security check failed.', 'size-chart-for-woocommerce' ) );
			}
			...
			if ( scfw_fs()->is__premium_only() ) {
				if ( scfw_fs()->can_use_premium_code() ) {
					...
					$size_chart_table_row_odd_color 	= filter_input( INPUT_POST, 'size-chart-table-row-odd-color', FILTER_SANITIZE_FULL_SPECIAL_CHARS );
					...
					$size_chart_button_class 		= filter_input( INPUT_POST, 'size-chart-button-class', FILTER_SANITIZE_SPECIAL_CHARS );
					$size_chart_size 				= filter_input( INPUT_POST, 'size-chart-size', FILTER_SANITIZE_SPECIAL_CHARS );
					...
					$this->size_chart_settings['size-chart-size']            = $size_chart_size;

					// Custom CSS Setting (Pro)
					...
					// User Role Setting
					$scsf_user_role = filter_input( INPUT_POST, 'scsf_user_role', FILTER_SANITIZE_SPECIAL_CHARS, FILTER_REQUIRE_ARRAY);

					$this->size_chart_settings['user_roles'] = $scsf_user_role;
				}
			}
			...
			?>
            <div class="updated">
                <p><strong><?php esc_html_e( 'Settings saved.', 'size-chart-for-woocommerce' ); ?></strong></p>
            </div>
			<?php
		}
	}
	...
	public function scfw_size_chart_product_and_size_chart_save_callback( $post_id ) {
		...
		if ( isset( $size_chart_inner_nonce ) && wp_verify_nonce( $size_chart_inner_nonce, 'size_chart_inner_custom_box' ) ) {
			...
			$chart_table       = filter_input( INPUT_POST, 'chart-table', FILTER_SANITIZE_FULL_SPECIAL_CHARS, FILTER_FLAG_NO_ENCODE_QUOTES );
	...
	public function scfw_size_chart_add_meta_box_callback( $post_type ) {
		...
		if ( in_array( $post_type, $post_types_chart, true ) ) {
			...
			if ( scfw_fs()->is__premium_only() && scfw_fs()->can_use_premium_code() ) {
				...
				add_meta_box( 'size-chart-shortcode',
					__( 'Shortcode', 'size-chart-for-woocommerce' ),
					array(
						$this,
						'scfw_size_chart_shortcode_callback__premium_only'
					),
			...
			add_meta_box( 'chart-settings',
				__( 'Size Chart Settings', 'size-chart-for-woocommerce' ),
				array(
					$this,
					'scfw_size_chart_meta_box_content_callback'
				),
				$this->get_plugin_post_type_name(),
				'advanced',
				'high'
			...
			add_meta_box( 'additional-chart',
				__( 'Search/Select Size Chart', 'size-chart-for-woocommerce' ),
				array(
					$this,
					'scfw_size_chart_select_chart_callback'
				),
				'product',
				'side',
				'default'
			...
			add_meta_box( 'chart-assign-category',
				__( 'Assign Category', 'size-chart-for-woocommerce' ),
				array(
					$this,
					'scfw_size_chart_assign_category_callback'
				),
				$this->get_plugin_post_type_name(),
				'side',
				'default'
			...
			add_meta_box( 'chart-assign-tag',
				__( 'Assign Tag', 'size-chart-for-woocommerce' ),
				array(
					$this,
					'scfw_size_chart_assign_tag_callback'
				),
				$this->get_plugin_post_type_name(),
				'side',
				'default'
            ...
            // Meta box to List of assign tag.
			add_meta_box( 'chart-assign-attributes',
                __( 'Assign Attributes', 'size-chart-for-woocommerce' ),
                array(
                    $this,
                    'scfw_size_chart_assign_attributes_callback'
                ),
                $this->get_plugin_post_type_name(),
                'side',
                'default'
			...
			add_meta_box( 'chart-assign-product',
				__( 'Assign Product', 'size-chart-for-woocommerce' ),
				array(
					$this,
					'scfw_size_chart_assign_product_callback'
				),
				$this->get_plugin_post_type_name(),
				'side',
				'default'
	...
	/**
	 *  Meta Box content for size chart shortcode
	 *
	 * @since      1.0.0
	 */
	public function scfw_size_chart_shortcode_callback__premium_only() {
		$current_size_chart_id  = get_the_ID();
		?>
		<div id="size-chart-meta-fields">
			<p><?php esc_html_e( 'You can use below shortcode to place this size chart where you want.', 'size-chart-for-woocommerce' ); ?></p>
			<input type="text" class="scfw-copy-shortcode" value='[scfw_size_chart id="<?php echo esc_attr($current_size_chart_id); ?>"]' readonly="readonly" />
			<div class="scfw-after-copy-text">
				<span class="dashicons dashicons-yes-alt"></span><?php esc_html_e( 'Shortcode  Copied to Clipboard!', 'size-chart-for-woocommerce' ); ?>
			</div>
		</div>
		<?php
	}
	...
	public function scfw_size_chart_duplicate_post_callback() {
		...
		if ( ( isset( $get_request_get ) && ! empty( $get_request_get ) ) ) {
			...
			wp_redirect(
				admin_url(
					'post.php?action=edit&post=' . $clone_post_id
				)
			);
			exit;
		} else {
			wp_die(
				esc_html__( 'could not find post: ' . $size_chart_id , 'size-chart-for-woocommerce' )
			);
		}
	}
	...
	public function scfw_size_chart_preview_dialog_box_callback() {
        ...
        <div id="wait" style="display:none;">
            <img src="<?php echo esc_url( SCFW_PLUGIN_URL . 'admin/images/loader.gif' ); ?>" width="64" height="64" alt="<?php esc_attr_e( 'loader', 'size-chart-for-woocommerce' ); ?>"/>
	...
	public function scfw_size_chart_manage_column_callback( $column ) {
		...
		switch ( $column ) {
			case 'product_assign' :
                ...
                if ( isset( $assigned_product_ids ) && !empty( $assigned_product_ids ) ) {
                	foreach( $assigned_product_ids as $assigned_product_id ) {
                	 	printf(
                            wp_kses_post( "$sep<a href='%s'>%s</a>" ),
                            esc_url( get_edit_post_link( $assigned_product_id ) ),
                            wp_kses_post( get_the_title( $assigned_product_id ) )
                        );
						$sep = ', ';
                	}
                } else {
						...
						echo sprintf(
							...
							esc_url(
								wp_nonce_url(
									...
									'scfw_size_chart_duplicate_post_callback'
								)
							),
			...
			case 'size_chart_shortcode' :
				?>
				<input type="text" class="scfw-copy-shortcode" value='[scfw_size_chart id="<?php echo esc_attr($post->ID) ?>"]' readonly="readonly">
				<div class="scfw-after-copy-text">
					<span class="dashicons dashicons-yes-alt"></span><?php esc_html_e('Shortcode  Copied to Clipboard!', 'size-chart-for-woocommerce') ?>
				</div>
				<?php
	...
	public function scfw_size_chart_remove_row_actions_callback( $actions ) {
        ...
        if( $this->get_plugin_post_type_name() === $post->post_type ) {
            ...
            $actions['previewing'] = sprintf( '<a id="%s" href="javascript:void(0);" class="preview_chart" rel="permalink">%s</a>',
                esc_attr( $post->ID ),
                esc_attr__( 'Preview', 'size-chart-for-woocommerce' )
            );
            $actions['clone'] = sprintf( '<a href="%s" rel="permalink">%s</a>',
                esc_url(
                    wp_nonce_url(
                        add_query_arg(
                            array(
	...
	/**
	 * Size chart - add closing div for size chart posts.
	 * 
	 * @since 1.0.0
	 */
	public function scfw_size_chart_posts_close_div() {
		global $typenow;
		$get_post_type = filter_input( INPUT_GET, 'post_type', FILTER_SANITIZE_SPECIAL_CHARS );
		if ( isset( $get_post_type ) && $get_post_type === 'size-chart' || $this->get_plugin_post_type_name() === $typenow ) {
			?>
	...
	public function scfw_size_chart_product_assign_callback() {
		...
		if ( 0 !== count( $response_array['found_products'] ) ) {
			...
			$response_array['load_pagination'] = scfw_size_chart_pagination_html( $wp_posts_query, $post_id, $post_per_page, false );
		}
	...
	public function scfw_size_chart_product_meta_query_argument() {
        ...
        $args = array(
		    ...
		    'fields' => 'ids',
		    'posts_per_page' => -1 // to retrieve all the matching products
		);
		// phpcs:enable
            ...
            <div class="chart-content">
                <div class="chart-content-list">
					<?php if ( 'tshirt-shirt' === $template ) { ?>
                        <p><?php esc_html_e( 'To choose the correct size for you, measure your body as follows:', 'size-chart-for-woocommerce' ); ?></p>
                        <ul>
                            <li>
                                <strong><?php esc_html_e( 'Chest : ', 'size-chart-for-woocommerce' ); ?></strong>
								<?php esc_html_e( 'Measure around the fullest part, place the tape close under the arms and make sure the tape is flat across the back.', 'size-chart-for-woocommerce' ); ?>
                            </li>
                        </ul>
					<?php } elseif ( 'womens-tshirt' === $template ) { ?>
                        <ul>
                            <li>
                                <strong><?php esc_html_e( 'Chest : ', 'size-chart-for-woocommerce' ); ?></strong>
								<?php esc_html_e( 'Measure under your arms, around the fullest part of the your chest.', 'size-chart-for-woocommerce' ); ?>
                            </li>
                            <li>
                                <strong><?php esc_html_e( 'Waist : ', 'size-chart-for-woocommerce' ); ?></strong>
								<?php esc_html_e( 'Measure around your natural waistline, keeping the tape a bit loose.', 'size-chart-for-woocommerce' ); ?>
                            </li>
                        </ul>
					...
					<?php } elseif ( 'womens-dress' === $template ) { ?>
                        <ul>
                            ...
                            <li>
                                <strong><?php esc_html_e( 'Hips : ', 'size-chart-for-woocommerce' ); ?></strong>
								<?php esc_html_e( 'Measure around the fullest part of your body at the top of your leg.', 'size-chart-for-woocommerce' ); ?>
                            </li>
                        </ul>
					<?php } elseif ( 'jeans-trouser' === $template ) { ?>
                        ...
                        <ul>
                            <li>
                                <strong><?php esc_html_e( 'Waist : ', 'size-chart-for-woocommerce' ); ?></strong>
								<?php esc_html_e( 'Measure around natural waistline.', 'size-chart-for-woocommerce' ); ?>
                            </li>
                            <li>
                                <strong><?php esc_html_e( 'Inside leg : ', 'size-chart-for-woocommerce' ); ?></strong>
								<?php esc_html_e( 'Measure from top of inside leg at crotch to ankle bone.', 'size-chart-for-woocommerce' ); ?>
                            </li>
                        </ul>
					<?php } elseif ( 'womens-jeans' === $template ) { ?>
                        <ul>
                            <li>
                                <strong><?php esc_html_e( 'Waist : ', 'size-chart-for-woocommerce' ); ?></strong>
								<?php esc_html_e( 'Measure around your natural waistline,keeping the tape bit loose.', 'size-chart-for-woocommerce' ); ?>
                            </li>
                            <li>
                                <strong><?php esc_html_e( 'Hips : ', 'size-chart-for-woocommerce' ); ?></strong>
								<?php esc_html_e( 'Measure around the fullest part of your body at the top of your leg.', 'size-chart-for-woocommerce' ); ?>
                            ...
                            <li>
                                <strong><?php esc_html_e( 'Inseam : ', 'size-chart-for-woocommerce' ); ?></strong>
								<?php esc_html_e( 'Wearing pants that fit well, measure from the crotch seam to the bottom of the leg.', 'size-chart-for-woocommerce' ); ?>
                            </li>
                        </ul>
					...
					<?php } elseif ( 'women-cloth' === $template ) { ?>
                        <ul>
                            <li>
                                <strong><?php esc_html_e( 'Chest : ', 'size-chart-for-woocommerce' ); ?></strong>
								<?php esc_html_e( 'Measure around the fullest part of the bust, keeping the tape parallel to the floor.', 'size-chart-for-woocommerce' ); ?>
                            </li>
                            <li>
                                <strong><?php esc_html_e( 'Waist : ', 'size-chart-for-woocommerce' ); ?></strong>
								<?php esc_html_e( 'Measure around the narrowest point, keeping the tape parallel to the floor.', 'size-chart-for-woocommerce' ); ?>
                            </li>
                            <li>
                                <strong><?php esc_html_e( 'Hip : ', 'size-chart-for-woocommerce' ); ?></strong>
								<?php esc_html_e( 'Stand with feet together and measure around the fullest point of the hip, keep the tape parallel to the floor.', 'size-chart-for-woocommerce' ); ?>
                            </li>
                            <li>
                                <strong><?php esc_html_e( 'Inseam : ', 'size-chart-for-woocommerce' ); ?></strong>
								<?php esc_html_e( 'Measure inside length of leg from your crotch to the bottom of ankle.', 'size-chart-for-woocommerce' ); ?>
                            </li>
                        </ul>
					<?php } ?>
                </div>
                <div class="chart-content-image">
					<?php if ( 'tshirt-shirt' === $template ) { ?>
                        <img class="alignnone size-medium alignright" src="<?php echo esc_url( SCFW_PLUGIN_URL . 'admin/images/default-chart/mens-tshirts-and-polo-shirts.jpg' ); ?>" alt="<?php esc_attr_e( 'tshirt-shirt-chart', 'size-chart-for-woocommerce' ); ?>" width="300" height="300"/>
					<?php } elseif ( 'womens-tshirt' === $template ) { ?>
                        <img class="alignnone size-medium alignright" src="<?php echo esc_url( SCFW_PLUGIN_URL . 'admin/images/default-chart/women-t-shirt-top.png' ); ?>" alt="<?php esc_attr_e( 'womens-tshirt', 'size-chart-for-woocommerce' ); ?>" width="300" height="300"/>
					<?php } elseif ( 'mens-shirts' === $template ) { ?>
                        <img class="alignnone size-medium alignright" src="<?php echo esc_url( SCFW_PLUGIN_URL . 'admin/images/default-chart/mens-shirts.jpg' ); ?>" alt="<?php esc_attr_e( 'mens-shirts', 'size-chart-for-woocommerce' ); ?>" width="300" height="300"/>
					<?php } elseif ( 'womens-dress' === $template ) { ?>
                        <img class="alignnone size-medium alignright" src="<?php echo esc_url( SCFW_PLUGIN_URL . 'admin/images/default-chart/women-dress-size-chart.png' ); ?>" alt="<?php esc_attr_e( 'womens-dress-chart', 'size-chart-for-woocommerce' ); ?>" width="300" height="300"/>
					<?php } elseif ( 'jeans-trouser' === $template ) { ?>
                        <img class="alignnone size-medium alignright" src="<?php echo esc_url( SCFW_PLUGIN_URL . 'admin/images/default-chart/mens-jeans-and-trousers.jpg' ); ?>" alt="<?php esc_attr_e( 'jeans-chart', 'size-chart-for-woocommerce' ); ?>" width="300" height="300"/>
					<?php } elseif ( 'womens-jeans' === $template ) { ?>
                        <img class="alignnone size-medium alignright" src="<?php echo esc_url( SCFW_PLUGIN_URL . 'admin/images/default-chart/women-jeans-size-chart.png' ); ?>" alt="<?php esc_attr_e( 'womens-jeans-chart', 'size-chart-for-woocommerce' ); ?>" width="300" height="300"/>
					<?php } elseif ( 'mens-waistcoats' === $template ) { ?>
                        <img class="alignnone size-medium alignright" src="<?php echo esc_url( SCFW_PLUGIN_URL . 'admin/images/default-chart/mens-waistcoats.jpg' ); ?>" alt="<?php esc_attr_e( 'mens-waistcoats', 'size-chart-for-woocommerce' ); ?>" width="300" height="300"/>
					<?php } elseif ( 'women-cloth' === $template ) { ?>
                        <img class="alignnone size-medium alignright" src="<?php echo esc_url( SCFW_PLUGIN_URL . 'admin/images/default-chart/cloth_size_chart.png' ); ?>" alt="<?php esc_attr_e( 'cloth-chart', 'size-chart-for-woocommerce' ); ?>" width="300" height="300"/>
					<?php } elseif ( 'men-shoes' === $template ) { ?>
                        <img class="alignnone size-medium alignright" src="<?php echo esc_url( SCFW_PLUGIN_URL . 'admin/images/default-chart/mens-shoes-size-chart.png' ); ?>" alt="<?php esc_attr_e( 'mens-shoe-chart', 'size-chart-for-woocommerce' ); ?>" width="300" height="300"/>
					<?php } elseif ( 'women-shoes' === $template ) { ?>
                        <img class="alignnone size-medium alignright" src="<?php echo esc_url( SCFW_PLUGIN_URL . 'admin/images/default-chart/women-shoes-size-image.jpg' ); ?>" alt="<?php esc_attr_e( 'womens-shoe-chart', 'size-chart-for-woocommerce' ); ?>" width="300" height="300"/>
					<?php } ?>
                </div>
            </div>
        </div>
    ...
    public function scfw_size_chart_export_data_callback(){
        ...
        foreach ($files as $csv_file) {
            unlink($csv_file); // phpcs:ignore
        }

        //Save new data to JSON file
        file_put_contents( $save_path.$file_name, $json_data ); // phpcs:ignore

        return wp_send_json_success( array( 'message' => esc_html__( 'Data has been Exported!', 'size-chart-for-woocommerce' ), 'download_path' => $download_path.$file_name ));
    }
    ...
    public function scfw_size_chart_import_data_callback(){
        ...
        if ( empty( $import_file ) ) {
            wp_send_json_error( array( 'message' => esc_html__( 'Please upload a file to import', 'size-chart-for-woocommerce' ) ) );
        }
        ...
        if ( $extension !== 'json' ) {
            wp_send_json_error( array( 'message' => esc_html__( 'Please upload a valid .json file', 'size-chart-for-woocommerce' ) ) );
        }
        WP_Filesystem();
        ...
        return wp_send_json_success( array( 'chart_data'=> $file_data, 'message' => esc_html__( 'Data has been Imported! Reload page in moment.', 'size-chart-for-woocommerce' ) ) );
    }

    /**
	 * Export size chart settings action.
	 * 
	 * @since 1.0.0
	 */
    public function scfw_export_settings_action__premium_only(){
        //Check ajax nonce reference
        check_ajax_referer('size_chart_for_wooocmmerce_nonoce', 'security');

        // To ignore slow meta query warnings in phpcs.
        // phpcs:disable
        $scfw_args     = array(
			'post_type'     => $this->get_plugin_post_type_name(),
			'post_status'   => 'publish',
		...
		// phpcs:enable
		$scfw_chart_ids = new WP_Query( $scfw_args );
        $final_array = array();
        foreach( $scfw_chart_ids->posts as $size_chart_id ){
            ...
            $size_chart_title = get_the_title( $size_chart_id );
            $size_chart_slug = get_post_field('post_name', $size_chart_id );
            $size_chart_content = scfw_size_chart_get_the_content( $size_chart_id );

            $chart_array['sc_title'] = $size_chart_title ? $size_chart_title : '';
            $chart_array['sc_slug'] = $size_chart_slug ? $size_chart_slug : '';
            $chart_array['sc_content'] = $size_chart_content ? $size_chart_content : '';

            foreach ( $default_size_chart_meta_data as $key => $values ) {
                foreach ( $values as $value ) {
                    if ( 'default' === $value || '_edit_lock' === $key || '_edit_last' === $key ) {
                        continue;
                    }
                    $chart_array[$key] = $value;
                }
            }
            ...
            if ( !empty( $wp_posts_query->posts ) ) {
                foreach( $wp_posts_query->posts as $product_id ) {
                    ...
                    $valid_product = false;
                    if( is_array( $data_size_chart ) ) {
                        $valid_product = in_array($size_chart_id, $data_size_chart, true);
                    } else {
                        $valid_product = ( $size_chart_id === $data_size_chart ) ? true : false;
                    }
                    if( true === $valid_product ) {
                        $assigned_product_ids[] = $product_id;
                    }
            ...
            $chart_array['product_ids'] = $assigned_product_ids;
            $final_array[$size_chart_id] = $chart_array;
        }
        
        $json_data = wp_json_encode($final_array);
        $file_name = 'scfw_export_'.time().'.json';

        $path_data = wp_get_upload_dir();
        $save_path = $path_data['basedir'].'/scfw_files/';
        $download_path = $path_data['baseurl'].'/scfw_files/';
    ...
    /**
	 * Import size chart settings action.
	 * 
	 * @since 1.0.0
	 */
    public function scfw_import_settings_action__premium_only(){
    	//Check ajax nonce reference
        check_ajax_referer('size_chart_for_wooocmmerce_nonoce', 'security');
        ...
        if( !empty( $file_data ) ) {
            ...
            $exclude_keys = array( 'sc_title', 'sc_slug', 'sc_content', 'product_ids' );
            foreach( $file_data as $sc_data ){
                
                if( isset( $sc_data['sc_slug'] ) && !empty( $sc_data['sc_slug'] ) ) {
                    $method_data = get_page_by_path( $sc_data['sc_slug'], OBJECT, $this->get_plugin_post_type_name()); // phpcs:ignore
                    $method_id = $method_data->ID;
                } else {
                    $method_data = get_page_by_title( $sc_data['sc_title'], OBJECT, $this->get_plugin_post_type_name()); // phpcs:ignore
                    $method_id = $method_data->ID;
                }
                
                if( is_null($method_id) || $method_id <= 0 || empty($method_id) ){
                    $import_args  = array(
                        'post_status'   => 'publish',
                        'post_type'     => $this->get_plugin_post_type_name(),
                        'post_title'    => $sc_data['sc_title'],
                        'post_name'     => $sc_data['sc_slug'],
                        'post_content'  => $sc_data['sc_content'],
                    );
                    $method_id = wp_insert_post( $import_args );
                } else {
                    $import_args  = array(
                        'ID'          => $method_id,
                        'post_status' => 'publish',
                    ...
                    $method_id = wp_update_post( $import_args );
                }
                
                if ( '' !== $method_id && 0 !== $method_id && $method_id > 0 ) {
                    foreach( $sc_data as $sc_data_k => $sc_data_v ){
                        if( in_array( $sc_data_k, $exclude_keys, true ) ){
                            continue;
                        }
                        if( 'chart_country' === $sc_data_k ) {
                            if ( isset( $sc_data_v ) && ! empty( $sc_data_v ) ) {
                                if(is_serialized( $sc_data_v )){
                                    $sc_data_v = maybe_unserialize($sc_data_v);
                                }
                            }
                        }
                        update_post_meta( $method_id, $sc_data_k, $sc_data_v );
                    }
                }
                //import in product ids for size chart
                if( isset( $sc_data['product_ids'] ) && !empty( $sc_data['product_ids'] ) ){
                    foreach( $sc_data['product_ids'] as $pid ){
                        
                        $size_chart_id_for_prod = scfw_size_chart_get_product($pid);

                        //if data not array then convert it to array first
                        if( !empty($size_chart_id_for_prod) && !is_array($size_chart_id_for_prod) ){
                            $size_chart_id_for_prod = str_replace( '[', '', str_replace( ']', '', $size_chart_id_for_prod ) );
                            $size_chart_id_for_prod = explode( ',', $size_chart_id_for_prod );
                        }
                        
                        //remove deleted chaart id from product meta
                        if( !empty($size_chart_id_for_prod) ) {
                            foreach( $size_chart_id_for_prod as $pk => $post_id ){
                                if( 'publish' !== get_post_status ( $post_id ) ){
                                    unset( $size_chart_id_for_prod[$pk] );
                                }
                            }
                        }

                        //add new chart id if product meta contain it or not
                        if( !in_array( $method_id, $size_chart_id_for_prod, true ) ){   
                            array_push( $size_chart_id_for_prod, $method_id );
                        }

                        //make same format and store it in product meta
                        $size_chart_id_for_prod = implode( ',', $size_chart_id_for_prod );
                        $size_chart_id_for_prod = '['.$size_chart_id_for_prod.']';
                        update_post_meta( $pid, 'prod-chart', $size_chart_id_for_prod );
                    }
                }
            }
        } else {
            wp_send_json_error( array( 'message' => esc_html__( 'Blank .json file uploaded.', 'size-chart-for-woocommerce' ) ) );
        }

        return wp_send_json_success( array( 'message' => esc_html__( 'Data has been Imported!', 'size-chart-for-woocommerce' ) ) );
    }
			...
			if($posts_count >= 10){
				...
				<script type="text/javascript">
					jQuery(document).ready(function($) {
						...
						$('.page-title-action').addClass('upgrade-now');
					});
				</script>
				<div class="notice notice-warning is-dismissible">
					<p><?php echo esc_html_e('Usage limit reached: 10 size charts utilized. Unlock unlimited access by upgrading.', 'size-chart-for-woocommerce'); ?></p>
				</div>
				<?php
			}
			$mc = 1;
	...
	function restrict_add_new_size_chart_post() {
		...
		if ( isset( $scfw_post_type ) && 'size-chart' === $scfw_post_type  ) {
			...
			if( count($valid_size_charts) >= 10 ){
				wp_die( esc_html__('Usage limit reached: 10 size charts utilized. Unlock unlimited access by upgrading.', 'size-chart-for-woocommerce'), '', array('response' => 403));
			}
		}
	}
     ...
     * @since 1.0.0
     */
    public function scfw_admin_footer_review() {
    	$screen = get_current_screen();
		if ( false !== strpos( $screen->base, '_page_size-chart' ) || false !== strpos( $screen->base, '_page_scfw' ) ) {
			$url = '';
			if ( scfw_fs()->is__premium_only() ) {
				if ( scfw_fs()->can_use_premium_code() ) {
					$url = esc_url( 'https://www.thedotstore.com/woocommerce-advanced-product-size-charts/#reviews' );
				} else {
			...
			$html = sprintf( wp_kses( __( '<strong>We need your support</strong> to keep updating and improving the plugin. Please <a href="%1$s" target="_blank">help us by leaving a good review</a> :) Thanks!', 'size-chart-for-woocommerce' ), array(
	            'strong' => array(),
	            'a'      => array(
	                'href'   => array(),
	                'target' => 'blank',
