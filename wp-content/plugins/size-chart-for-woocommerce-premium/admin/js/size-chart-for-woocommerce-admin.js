...
(function($, window, document) {
	...
	var sizeChartScripts = {
		...
		loadPreviewSizeChart: function() {
			...
			$('a.preview_chart').click(function() {
                ...
                $('.inputtable tbody tr').each(function(){
                    var sarr = [];
                    $(this).find('td:not(td:last-child)').each(function(){
                        var data = $(this).find('input').val() ? $(this).find('input').val() : '';
                        sarr.push(data);
                    }); 
                    farr.push(sarr);
                });

                chart_color.scfw_header_bg_color = $('#scfw_header_bg_color').val();
                chart_color.scfw_text_color = $('#scfw_text_color').val();
                chart_color.scfw_odd_row_bg_color = $('#scfw_odd_row_bg_color').val();
                chart_color.scfw_odd_text_color = $('#scfw_odd_text_color').val();
                chart_color.scfw_even_row_bg_color = $('#scfw_even_row_bg_color').val();
                chart_color.scfw_even_text_color = $('#scfw_even_text_color').val();
                chart_color.scfw_enable_table_hover = $('#scfw_enable_table_hover').val();
                chart_color.scfw_table_hover_bg_color = $('#scfw_table_hover_bg_color').val();

                chart_border.scfw_border_hb_style = $('#scfw_border_hb_style').val();
                chart_border.scfw_border_vb_style = $('#scfw_border_vb_style').val();
                chart_border.scfw_border_hw = $('#scfw_border_hw').val();
                chart_border.scfw_border_vw = $('#scfw_border_vw').val();
                chart_border.scfw_border_color = $('#scfw_border_color').val();
				...
				$.ajax({
					...
					beforeSend: function() {
						...
						if (1 === response.success) {
							...
							/* <fs_premium_only> */
							// Size chart preview table cell hover effect script
        ...
        sizeChartImportExportModule: function(){
            ...
            /* <fs_premium_only> */
            // Export AJAX call
            $('#scfw_export_settings').click(function(){
                var action = $('input[name="scfw_export_action"]').val();
                var $this = $(this);
                $('.thedotstore-main-table table').block({
                    message: null,
                    overlayCSS: {
                ...
                if( action ){
                    ...
                    $.ajax({
                        ...
                        success: function (response) {
                            $('.thedotstore-main-table table').unblock();
                            var p_node = document.createElement( 'p' );
                            setAllAttributes( p_node, {
                                'style': 'color: green;',
                            } );
                            var text_node = document.createTextNode( response.data.message );
                            p_node.appendChild( text_node );
                            $(p_node).insertAfter($('.export_settings_container .scfw_button_container'));
                            if( response.data.download_path ){
                                var link = document.createElement('a');
                                document.body.appendChild(link);
                                link.href = response.data.download_path;
            ...
            //Import Ajax call
            $('#scfw_import_settings').click(function(e){
                e.preventDefault();
                var action = $('input[name="scfw_import_action"]').val();
                var $this = $(this);
                $('.thedotstore-main-table table').block({
                    message: null,
                    overlayCSS: {
                ...
                if( action ){
                    ...
                    $.ajax({
                        ...
                        success: function(response){
                            ...
                            var text_node = document.createTextNode( response.data.message );
                            p_node.appendChild( text_node );
                            $(p_node).insertAfter($('.import_settings_container .scfw_button_container'));
                            setTimeout( function(){
                                p_node.remove();
                                $this.attr('disabled', null);
                            }, 3000 );
	...
	$(document).ready(function(){
        ...
        // Chart table font size JS
        $('input[name="chart-table-font-sizes"]').change(function(){
            var value = $(this).val();
            $('#chart-table-font-size').val(value);
        });
        $('#chart-popup-icon').on('input', function(){
            var value = $(this).val();
            $('input[name="chart-table-font-sizes"]').prop('checked', false);
            if( '' !== value ){
                $('input[name="chart-table-font-sizes"]').each(function( e, val ){
                    if( $(val).val() === value ){
                        $('input[name="chart-table-font-sizes"][value="'+value+'"]').prop('checked', true);
                    }
                });
            } else {
                $('input[name="chart-table-font-sizes"][value="s"]').prop('checked', true);
            }
        });
