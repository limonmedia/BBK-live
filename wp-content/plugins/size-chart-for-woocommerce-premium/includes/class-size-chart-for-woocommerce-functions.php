...
function scfw_size_chart_get_inline_styles_by_post_id( $post_id = 0, $table_style = '', $chart_table_font_size = '' ) {
	...
	if ( scfw_fs()->is__premium_only() ) {
		if ( scfw_fs()->can_use_premium_code() ) {
			$size_chart_title_color  = scfw_size_chart_get_title_color();

			if( empty($chart_table_font_size) ){
		    	$chart_table_font_size = scfw_size_chart_table_font_size__premium_only( $post_id );
		    }
		}
	}
	...
	if ( scfw_fs()->is__premium_only() ) {
		if ( scfw_fs()->can_use_premium_code() ) {
			if ( 'custom-style' === $table_style ) {
				$size_chart_table_head_color      = scfw_size_chart_get_table_head_color();
				$size_chart_table_head_font_color = scfw_size_chart_get_table_head_font_color();
				$size_chart_table_row_even_color  = scfw_size_chart_get_table_row_even_color();
				$size_chart_table_row_odd_color   = scfw_size_chart_get_table_row_odd_color();
				$size_chart_inline_style          .= "table#size-chart.custom-style tr th {background: {$size_chart_table_head_color};color: {$size_chart_table_head_font_color};} #size-chart.custom-style tr:nth-child(even) {background: {$size_chart_table_row_even_color}}#size-chart.custom-style td:nth-child(even) {background: none;}
                #size-chart.custom-style tr:nth-child(odd) {background: {$size_chart_table_row_odd_color}}
                #size-chart.custom-style td:nth-child(odd) {background: none;}
                .button-wrapper #chart-button, .button-wrapper .md-size-chart-btn {color: {$size_chart_title_color}}";
			}
            if ( 'advance-style' === $table_style ) {
                $chart_color       = scfw_size_chart_color_details__premium_only( $post_id );
                $chart_border      = scfw_size_chart_border_details__premium_only( $post_id );

                $size_chart_inline_style          .= "table#size-chart.advance-style tr th {background: {$chart_color['scfw_header_bg_color']};color: {$chart_color['scfw_text_color']};} 
            ...
            $size_chart_inline_style .= "table#size-chart tr th, table#size-chart tr td{font-size:{$table_font_size}}";

            $chart_table_hover = scfw_size_chart_table_hover__premium_only( $post_id );
            if ( 'yes' === $chart_table_hover['scfw_enable_table_hover'] && 'advance-style' === $table_style ) {
            	$table_hover_bg_color = $chart_table_hover['scfw_table_hover_bg_color'];
            	$opacity = 0.5;
            	...
            	$size_chart_inline_style .= ".scfw-chart-table tr:hover td, .scfw-chart-table tr:hover th{background-color: {$rgbaColor}!important;}
            ...
            $size_chart_inline_style .= scfw_size_chart_get_custom_css();
		}
	}
...
function scfw_size_chart_get_chart_table( $chart_table, $chart_id, $table_style = '' ) {
	...
	if (!empty($chart_table) && array_filter($chart_table)) {
	    $i = 0;
	    $tableOpen = false; // Flag to track if a table is open

	    foreach ($chart_table as $chart) {
	        $hasNewTableValue = false; // Flag to check if a new table value exists in the current chart
	        $hasNewTitleValue = false; // Flag to check if a title value exists in the current chart
	        $titleValue = '';

	        foreach ($chart as $value) {
	            if (substr($value, 0, 3) === '***' && substr($value, -3) === '***') {
	                $hasNewTitleValue = true;
	                $titleValue = trim($value, '*'); // Remove ** signs from the starting and ending of the value
	                break;
	            }
	        ...
	        if ($hasNewTableValue || $hasNewTitleValue) {
	            if ($tableOpen) {
	                echo "</table>"; // Close the previous table if any
	            }
	        ...
	        if (array_filter($chart)) {
	            $skipArray = false; // Flag to skip the array
	            ...
	            if ($skipArray) {
	                continue; // Skip the current array and move to the next iteration
	            }
	            ...
	            foreach ($chart as $value) {
	                // If data available.
	                if (!empty($value)) {
	                    $value = trim($value, '*'); // Remove ** signs from the starting and ending of the value
	    ...
	    if ($tableOpen) {
	        echo "</table>"; // Close the last table if any
	    }
	}
...
function scfw_size_chart_color_details__premium_only( $size_chart_id ){
    $color_array = array();

    $scfw_header_bg_color   = get_post_meta( $size_chart_id, 'scfw_header_bg_color', true );
    $scfw_even_row_bg_color = get_post_meta( $size_chart_id, 'scfw_even_row_bg_color', true );
    $scfw_odd_row_bg_color  = get_post_meta( $size_chart_id, 'scfw_odd_row_bg_color', true );
    $scfw_text_color        = get_post_meta( $size_chart_id, 'scfw_text_color', true );
    $scfw_even_text_color   = get_post_meta( $size_chart_id, 'scfw_even_text_color', true );
    $scfw_odd_text_color    = get_post_meta( $size_chart_id, 'scfw_odd_text_color', true );

	$color_array['scfw_header_bg_color']    = ( isset( $scfw_header_bg_color ) && ! empty( $scfw_header_bg_color ) ) ? $scfw_header_bg_color : '#ffffff';
	$color_array['scfw_even_row_bg_color']  = ( isset( $scfw_even_row_bg_color ) && ! empty( $scfw_even_row_bg_color ) ) ? $scfw_even_row_bg_color : '#ffffff';
	$color_array['scfw_odd_row_bg_color']   = ( isset( $scfw_odd_row_bg_color ) && ! empty( $scfw_odd_row_bg_color ) ) ? $scfw_odd_row_bg_color : '#ffffff';

    $color_array['scfw_text_color']         = ( isset( $scfw_text_color ) && ! empty( $scfw_text_color ) ) ? $scfw_text_color : '#000000';
    $color_array['scfw_even_text_color']    = ( isset( $scfw_even_text_color ) && ! empty( $scfw_even_text_color ) ) ? $scfw_even_text_color : '#000000';
    $color_array['scfw_odd_text_color']     = ( isset( $scfw_odd_text_color ) && ! empty( $scfw_odd_text_color ) ) ? $scfw_odd_text_color : '#000000';

	return $color_array;
}

function scfw_size_chart_border_details__premium_only( $size_chart_id ){
    $border_array = array();

    $scfw_border_hb_style   = get_post_meta( $size_chart_id, 'scfw_border_hb_style', true );
    $scfw_border_hw         = get_post_meta( $size_chart_id, 'scfw_border_hw', true );
    $scfw_border_vb_style   = get_post_meta( $size_chart_id, 'scfw_border_vb_style', true );
    $scfw_border_vw         = get_post_meta( $size_chart_id, 'scfw_border_vw', true );
    $scfw_border_color      = get_post_meta( $size_chart_id, 'scfw_border_color', true );

    $border_array['scfw_border_hb_style']   = ( isset( $scfw_border_hb_style ) && ! empty( $scfw_border_hb_style ) ) ? $scfw_border_hb_style : 'solid';
    $border_array['scfw_border_hw']         = ( isset( $scfw_border_hw ) && ! empty( $scfw_border_hw ) ) ? $scfw_border_hw : 1;
    $border_array['scfw_border_vb_style']   = ( isset( $scfw_border_vb_style ) && ! empty( $scfw_border_vb_style ) ) ? $scfw_border_vb_style : 'solid';
    $border_array['scfw_border_vw']         = ( isset( $scfw_border_vw ) && ! empty( $scfw_border_vw ) ) ? $scfw_border_vw : 1;
    $border_array['scfw_border_color']      = ( isset( $scfw_border_color ) && ! empty( $scfw_border_color ) ) ? $scfw_border_color : '#000000';

    return $border_array;
}

function scfw_size_chart_table_hover__premium_only( $size_chart_id ){
    $table_hover_array = array();

    $scfw_enable_table_hover   = get_post_meta( $size_chart_id, 'scfw_enable_table_hover', true );
    $scfw_table_hover_bg_color = get_post_meta( $size_chart_id, 'scfw_table_hover_bg_color', true );

    $table_hover_array['scfw_enable_table_hover'] 	= ( isset( $scfw_enable_table_hover ) && ! empty( $scfw_enable_table_hover ) ) ? $scfw_enable_table_hover : 'no';
    $table_hover_array['scfw_table_hover_bg_color'] = ( isset( $scfw_table_hover_bg_color ) && ! empty( $scfw_table_hover_bg_color ) ) ? $scfw_table_hover_bg_color : '#d5d4d4';

    return $table_hover_array;
}
...
/**
 * Get the chart country.
 *
 * @param int $size_chart_id size chart id.
 *
 * @return mixed|string a size chart country.
 */
function scfw_size_chart_country__premium_only( $size_chart_id ) {
	$chart_country = get_post_meta( $size_chart_id, 'chart_country', true );
    
    if ( isset( $chart_country ) && ! empty( $chart_country ) ) {
        if(is_serialized( $chart_country )){
            $chart_country = maybe_unserialize($chart_country);
        }
...
/**
 * Get the chart table font size.
 *
 * @param int $size_chart_id size chart id.
 *
 * @return mixed|string a font size.
 */
function scfw_size_chart_table_font_size__premium_only( $size_chart_id ) {
	$chart_font_size = get_post_meta( $size_chart_id, 'chart-table-font-size', true );
	if ( isset( $chart_font_size ) && ! empty( $chart_font_size ) ) {
...
/**
 * Get the chart popup position value.
 *
 * @param int $size_chart_id size chart id.
 *
 * @return mixed|string a popup position.
 */
function scfw_get_popup_postition__premium_only( $size_chart_id ) {
	$chart_popup_position = get_post_meta( $size_chart_id, 'popup-position', true );
	if ( isset( $chart_popup_position ) && ! empty( $chart_popup_position ) ) {
		return $chart_popup_position;
	}
}
 ...
 */
function scfw_size_chart_pagination_html( $size_chart_query, $current_post_id = 0, $posts_per_page = 10, $html = true ) {
	...
	if ( true === $html ) {
		...
		if ( $current_page !== 1 ) {
			...
			$pagination_html   .= scfw_size_chart_get_link_html( $first_page_number, $current_post_id, $posts_per_page, esc_html__( "<<", "size-chart-for-woocommerce" ), true, 'prev' );
		}

		// When to display "..." and the first page before it.
		if ( $first_link_in_the_middle >= 3 && $links_in_the_middle < $max_page ) {
			$pagination_html .= scfw_size_chart_get_link_html( 1, $current_post_id, $posts_per_page, esc_html__( "1", "size-chart-for-woocommerce" ) );
		...
		for ( $lopp_number = $first_link_in_the_middle; $lopp_number <= $last_link_in_the_middle; $lopp_number ++ ) {
			if ( $lopp_number === $current_page ) {
				...
				$pagination_html .= scfw_size_chart_get_link_html( $lopp_number, $current_post_id, $posts_per_page, esc_html( $lopp_number ) );
			}
		}
		...
		if ( $last_link_in_the_middle < $max_page ) {
			...
			$pagination_html .= scfw_size_chart_get_link_html( $max_page, $current_post_id, $posts_per_page, esc_html( $max_page ) );
		}

		// Arrow right (next page).
		if ( $current_page !== $last_link_in_the_middle ) {
			$next_page_number = $current_page + 1;
			$pagination_html  .= scfw_size_chart_get_link_html( $next_page_number, $current_post_id, $posts_per_page, esc_html__( ">>", "size-chart-for-woocommerce" ), true, 'next' );
		}
		// end HTML
		...
		if ( $current_page !== 1 ) {
			$first_page_number          = $current_page - 1;
			$pagination_array        [] = scfw_size_chart_get_link_html( $first_page_number, $current_post_id, $posts_per_page, __( "<<", "size-chart-for-woocommerce" ), false, 'prev' );
		}

		// When to display "..." and the first page before it.
		if ( $first_link_in_the_middle >= 3 && $links_in_the_middle < $max_page ) {
			$pagination_array [] = scfw_size_chart_get_link_html( 1, $current_post_id, $posts_per_page, esc_html__( "1", "size-chart-for-woocommerce" ), false );
		...
		for ( $lopp_number = $first_link_in_the_middle; $lopp_number <= $last_link_in_the_middle; $lopp_number ++ ) {
			if ( (int) $lopp_number === (int) $current_page ) {
				...
				$pagination_array [] = scfw_size_chart_get_link_html( $lopp_number, $current_post_id, $posts_per_page, esc_html( $lopp_number ), false );
			}
		}
		...
		if ( $last_link_in_the_middle < $max_page ) {
			...
			$pagination_array [] = scfw_size_chart_get_link_html( $max_page, $current_post_id, $posts_per_page, esc_html( $max_page ), false );
		}

		// Arrow right (next page).
		if ( $current_page !== $last_link_in_the_middle ) {
			$next_page_number          = $current_page + 1;
			$pagination_array       [] = scfw_size_chart_get_link_html( $next_page_number, $current_post_id, $posts_per_page, __( ">>", "size-chart-for-woocommerce" ), false, 'next' );
		}
 ...
 * @return string|array
 */
function scfw_size_chart_get_link_html( $page_number, $post_id, $posts_per_page, $page_text, $html = true, $class_name = '' ) {
	if ( true === $html ) {
		return sprintf(
...
if ( scfw_fs()->is__premium_only() ) {
	if ( scfw_fs()->can_use_premium_code() ) {
		/****** General Settings (Pro) start *****/

		/**
		 * Get the table head color from default setting.
		 *
		 * @return mixed|string color code value.
		 */
		function scfw_size_chart_get_table_head_color() {
			$size_chart_field_value = scfw_size_chart_get_setting( 'size-chart-table-head-color' );
			if ( isset( $size_chart_field_value ) && ! empty( $size_chart_field_value ) ) {
				return $size_chart_field_value;
			}
		 ...
		 * @return mixed|string color code value.
		 */
		function scfw_size_chart_get_table_head_font_color() {
			$size_chart_field_value = scfw_size_chart_get_setting( 'size-chart-table-head-font-color' );
			if ( isset( $size_chart_field_value ) && ! empty( $size_chart_field_value ) ) {
				return $size_chart_field_value;
			}
		 ...
		 * @return mixed|string color code value.
		 */
		function scfw_size_chart_get_table_row_even_color() {
			$size_chart_field_value = scfw_size_chart_get_setting( 'size-chart-table-row-even-color' );
			if ( isset( $size_chart_field_value ) && ! empty( $size_chart_field_value ) ) {
				return $size_chart_field_value;
			}
		 ...
		 * @return mixed|string color code value.
		 */
		function scfw_size_chart_get_table_row_odd_color() {
			$size_chart_field_value = scfw_size_chart_get_setting( 'size-chart-table-row-odd-color' );
			if ( isset( $size_chart_field_value ) && ! empty( $size_chart_field_value ) ) {
				return $size_chart_field_value;
			}
		...
		/****** General Settings (Pro) end *****/

		/****** Pop Up Settings (Pro) start *****/
		 ...
		 * @return mixed|string a string value.
		 */
		function scfw_size_chart_get_button_position() {
			$size_chart_field_value = scfw_size_chart_get_setting( 'size-chart-button-position' );
			if ( isset( $size_chart_field_value ) && ! empty( $size_chart_field_value ) ) {
				return $size_chart_field_value;
			}
		 ...
		 *
		 * @return mixed|string a color code value.
		 */
		function scfw_size_chart_get_title_color() {
			$size_chart_field_value = scfw_size_chart_get_setting( 'size-chart-title-color' );
			if ( isset( $size_chart_field_value ) && ! empty( $size_chart_field_value ) ) {
				return $size_chart_field_value;
			}
		 ...
		 * @return mixed|string a string value.
		 */
		function scfw_size_chart_get_button_class() {
			$size_chart_field_value = scfw_size_chart_get_setting( 'size-chart-button-class' );
			if ( isset( $size_chart_field_value ) && ! empty( $size_chart_field_value ) ) {
				return $size_chart_field_value;
			}
		...
		/****** Pop Up Settings (Pro) end *****/

		/****** Custom CSS Setting (Pro) start *****/
		/**
		 * Get the custom css from default setting.
		 *
		 * @return mixed|string a css string value.
		 */
		function scfw_size_chart_get_custom_css() {
			$size_chart_field_value = scfw_size_chart_get_setting( 'custom_css' );
			if ( isset( $size_chart_field_value ) && ! empty( $size_chart_field_value ) ) {
				return $size_chart_field_value;
			}
		...
		/**
		 * Get the user roles from default setting.
		 *
		 */
		function scfw_size_chart_get_user_roles() {
			$size_chart_field_value = scfw_size_chart_get_setting( 'user_roles' );
			if ( isset( $size_chart_field_value ) && ! empty( $size_chart_field_value ) ) {
				return $size_chart_field_value;
			}
...
function scfw_size_chart_check_empty_array($array) {
    ...
    return array_filter($array, function($item) {
        return !empty($item) || $item === 0; // Add additional checks if needed
    });
}