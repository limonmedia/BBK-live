...
if ( $is_preview || $is_template ) {
	...
	if ( $product->is_type( 'variable' ) ) {
		...
		$price         = floatval( apply_filters( 'yith_wcbm_advanced_badge_product_price', $product->get_price(), $product ) );
		$regular_price = floatval (apply_filters( 'yith_wcbm_advanced_badge_product_regular_price', $product->get_regular_price(), $product ) );
		
		$sale_percentage_float = 0;
		if ( 0.0 !== $regular_price ) {
