...
jQuery( function ($) {
	...
	var addRuleButtons           = $( '.post-php.post-type-ywcbm-badge-rule .page-title-action,.edit-php.post-type-ywcbm-badge-rule .page-title-action, .edit-php.post-type-ywcbm-badge-rule .yith-plugin-fw__list-table-blank-state__cta' ),
		...
		initBadgeSelect = function () {
			$('#_badge').attr('required', 'true');
		},
		...
		checkInputValidity       = function () {
			...
			if ( $input.is( 'select' ) && $input.hasClass( 'enhanced' ) ) {
				$input.parent().find( '.select2-container' ).css( { 'border-color': color, 'box-shadow': 'none' } );
			} else {
	...
	$( document ).on( 'change keyup', '#yith-wcbm-title, #_badge', checkInputValidity );
	$( '#yith-wcbm-title, #_badge' ).on( 'invalid', checkInputValidity );
