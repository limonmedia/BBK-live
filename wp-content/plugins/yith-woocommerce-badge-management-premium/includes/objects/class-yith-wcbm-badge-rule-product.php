				...
				if ( ! $this->is_product_excluded( $product->get_id() ) ) {
					switch ( $this->get_assign_to() ) {
						...
						case 'recent':
							...
							$product_publishing_date = get_post_timestamp( apply_filters('yith_wcbm_product_id_for_recent_rule', $product->get_id()), 'published' );
						...
						case 'back-order':
							...
							break;
						case 'low-stock':
							$valid = $product->managing_stock() && $product->get_stock_quantity() <= $this->get_low_stock_quantity() && $product->is_in_stock();
							break;
						case 'bestsellers':
