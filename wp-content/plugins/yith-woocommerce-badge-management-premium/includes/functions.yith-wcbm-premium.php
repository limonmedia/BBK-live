		...
		if ( null === $badges ) {
			...
			if ( ! $badges || ! get_transient( 'yith_wcbm_is_badges_list_consistent' ) ) {
				...
				if ( 200 === $status ) {
					...
					$api_url       = 'https://casper.yithemes.com/resources/yith-woocommerce-badge-management/badges/list.json';
					$response      = wp_remote_get( $api_url, $api_call_args );
					$response      = wp_remote_retrieve_body( $response );
					$response      = json_decode( $response, true );
					if ( is_array( $response ) && array_key_exists( 'badges', $response ) && $response['badges'] ) {
...
if ( ! function_exists( 'yith_wcbm_has_active_license' ) ) {
	...
	function yith_wcbm_has_active_license() {
		if ( class_exists( 'YITH\PluginUpgrade\Licences' ) ) {
			$licences = YITH\PluginUpgrade\Licences::instance();
			if ( is_callable( array( $licences, 'get_licence' ) ) ) {
				$licences_list  = $licences->get_licences();
				$plugin_licence = $licences_list[ YITH_WCBM_INIT ] ?? false;

				return $plugin_licence && $plugin_licence->is_activated();
			}
		}
	...
	function yith_wcbm_get_badges_placeholders( $product = false ) {
		$placeholders        = array(
			...
			'min_regular_price'    => array(
				'desc'  => __( 'Show the minimum regular price among the variations', 'yith-woocommerce-badges-management' ),
				'value' => '',
			),
			...
			'min_sale_price'       => array(
				'desc'  => __( 'Show the minimum sale price among the variations', 'yith-woocommerce-badges-management' ),
				'value' => '',
			),
			...
			'min_stock_quantity'   => array(
				'desc'  => __( 'Show the minimum available stock for the product variations', 'yith-woocommerce-badges-management' ),
				'note'  => __( 'Note: in product without stock management the badge will be not shown', 'yith-woocommerce-badges-management' ),
				'value' => '',
			),
			'max_stock_quantity'   => array(
				'value' => '',
			),
			'saved_amount'         => array(
			...
			'min_saved_amount'     => array(
				'desc'  => __( "Show the customer's minimum saving amount", 'yith-woocommerce-badges-management' ),
				'value' => '',
			),
			'max_saved_amount'     => array(
				'value' => '',
			),
			...
			'min_saved_percentage' => array(
				'desc'  => __( "Show the customer's minumum saving percentage", 'yith-woocommerce-badges-management' ),
				'value' => '',
			),
			'max_saved_percentage' => array(
				'value' => '',
			),
		);
		...
		if ( 'template' === $product ) {
			$placeholders_values = array(
				...
				'min_sale_price'       => '20' . $currency_symbol,
				'max_sale_price'       => '35' . $currency_symbol,
				'regular_price'        => '50' . $currency_symbol,
				'min_regular_price'    => '40' . $currency_symbol,
				'max_regular_price'    => '60' . $currency_symbol,
				'stock_quantity'       => '5',
				'min_stock_quantity'   => '3',
				'max_stock_quantity'   => '8',
				'saved_amount'         => '20' . $currency_symbol,
				'min_saved_amount'     => '10' . $currency_symbol,
				'max_saved_amount'     => '25' . $currency_symbol,
				'saved_percentage'     => '40%',
				'min_saved_percentage' => '30%',
				'max_saved_percentage' => '50%',
			);
		} else {
			...
			if ( $product ) {
				...
				if ( $is_variable ) {
					$variations_placeholders = array();
					foreach ( $product->get_children() as $child_id ) {
						$variation = wc_get_product( $child_id );
						if ( $variation ) {
							$variations_placeholders[] = yith_wcbm_get_badges_placeholders_values( $variation );
						}
					}

					foreach ( [ 'regular_price', 'sale_price', 'stock_quantity', 'saved_amount', 'saved_percentage' ] as $placeholder ) {
						$placeholders_values["min_{$placeholder}"] = count( array_column( $variations_placeholders, $placeholder ) ) > 0 ? min( array_column( $variations_placeholders, $placeholder ) ) : null;
						$placeholders_values["max_{$placeholder}"] = count( array_column( $variations_placeholders, $placeholder ) ) > 0 ? max( array_column( $variations_placeholders, $placeholder ) ) : null;

						if ( 1 === count( array_unique( array_column( $variations_placeholders, $placeholder ) ) ) ) {
							switch ( $placeholder ) {
								case 'regular_price':
									$regular_price = floatval( current( array_column( $variations_placeholders, $placeholder ) ) );
									break;
								case 'sale_price':
									$sale_price = floatval( current( array_column( $variations_placeholders, $placeholder ) ) );
									break;
								case 'stock_quantity':
									$stock = floatval( current( array_column( $variations_placeholders, $placeholder ) ) );
							}
						}
					}
				} else {
				...
				if ( apply_filters( 'yith_wcbm_placeholder_include_tax', true, $product ) ) {
                ...
                $precision = apply_filters( 'yith_wcbm_placeholder_round_precision', 0 );

				$placeholders_values['regular_price'] = abs( round( $regular_price , $precision) ) . $currency_symbol;

				if ( $sale_price && $regular_price ) {
					$placeholders_values['sale_price']       = abs(  round( $sale_price, $precision ) ) . $currency_symbol;
					$placeholders_values['saved_amount']     = abs( round( $regular_price - $sale_price, $precision ) ) . $currency_symbol;
					$placeholders_values['saved_percentage'] = abs( round( 100 / $regular_price * ( $regular_price - $sale_price ),$precision )  ) . '%';
				}

				if ( $stock ) {
					$placeholders_values['stock_quantity'] = $stock;
				}

				if ( ! $is_variable ) {
					foreach ( $placeholders_values as $placeholder => $value ) {
						$placeholders_values[ 'min_' . $placeholder ] = $value;
						$placeholders_values[ 'max_' . $placeholder ] = $value;
					}
				}
		...
		foreach ( $placeholders_values as $placeholder => $value ) {
			if ( array_key_exists( $placeholder, $placeholders ) && null !== $value ) {
...
if ( ! function_exists( 'yith_wcbm_has_unsolved_placeholders' ) ) {
	/**
	 * Return the number of unsolved placeholders
	 *
	 * @param string $text
	 * @return int
	 */
	function yith_wcbm_has_unsolved_placeholders( $text ) {
		return count(
			array_filter(
				array_map(
