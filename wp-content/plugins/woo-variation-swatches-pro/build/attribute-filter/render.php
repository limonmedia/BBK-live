<?php
/**
 * Attribute Filter Render Template.
 *
 * @global $attributes
 * @global $content
 * @global $block
...
<div <?php echo wp_kses_data( get_block_wrapper_attributes() ); ?>>
	<?php
	$attribute_taxonomy = wc_get_attribute( $attributes['attributeId'] );

	if ( !empty( $attribute_taxonomy ) ) {
		$terms               = get_terms( $attribute_taxonomy->slug );
		$attribute_type      = $attribute_taxonomy->type;
		$_chosen_attributes  = WC_Query::get_layered_nav_chosen_attributes();
		$selected_terms      = $_chosen_attributes[ $attribute_taxonomy->slug ]['terms'] ?? array();
		$display_limit       = 10;
		...
		echo '<ul class="filter-items' . esc_attr($display_limit_class) . '" data-attribute="' . esc_attr(wc_attribute_taxonomy_slug( $attribute_taxonomy->slug )) . '" data-query_type="' . esc_attr( $attributes['queryType'] ) . '"  data-select_type="' . esc_attr( $attributes['selectType'] ) . '">';

		foreach ( $terms as $attribute_term ) {
			$term_id                 = absint( $attribute_term->term_id );
			$name                    = esc_html( $attribute_term->name );
			$slug                    = esc_html( $attribute_term->slug );
			$count                   = absint( $attribute_term->count );
			$attribute               = woo_variation_swatches_pro()->get_frontend()->get_attribute_taxonomy_by_name( $attribute_term->taxonomy );
			$current_values          = $_chosen_attributes[ $attribute_taxonomy->slug ]['terms'] ?? array();
			$option_is_set           = in_array( $slug, $current_values, true );
			$filter_item_class       = sprintf( 'filter-item item-type-%1$s style-%2$s %3$s', esc_attr( $attribute_type ), woo_variation_swatches_pro()->get_option( 'shape_style' ), ( $option_is_set ) ? 'selected' : '' );
			$global_tooltip_type     = sanitize_text_field( woo_variation_swatches_pro()->get_frontend()->get_product_attribute_tooltip_type( $term_id ) );
			$global_tooltip_image_id = absint( woo_variation_swatches()->get_frontend()->get_product_attribute_tooltip_image_id( $term_id ) );
			$global_tooltip_text     = sanitize_text_field( woo_variation_swatches()->get_frontend()->get_product_attribute_tooltip_text( $term_id ) );
			...
			if ( woo_variation_swatches_pro()->get_frontend()->is_color_attribute( $attribute ) ) {
				$is_dual_color = woo_variation_swatches_pro()->get_frontend()->get_product_attribute_is_dual_color( $attribute_term );
				$primary_color = sanitize_hex_color( woo_variation_swatches_pro()->get_frontend()->get_product_attribute_primary_color( $attribute_term ) );

				if ( $is_dual_color ) {
					$secondary_color = sanitize_hex_color( woo_variation_swatches_pro()->get_frontend()->get_product_attribute_secondary_color( $attribute_term ) );
			...
			if ( woo_variation_swatches_pro()->get_frontend()->is_image_attribute( $attribute ) ) {
				$variation_image_id   = absint( woo_variation_swatches_pro()->get_frontend()->get_product_attribute_image( $attribute_term ) );
			...
			<li class="<?php echo esc_attr( $filter_item_class ); ?>"
				...
				<div class="filter-item-wrapper">
					<div
						class="<?php echo esc_attr( $filter_contents_class ); ?>" <?php echo wp_kses_post(  wc_implode_html_attributes( $html_attributes ) ); ?>>
		...
		if ( count( $terms ) > $display_limit ) {
			echo sprintf( '<li class="filter-item-more"><a style="font-size: small" href="#">%s</a></li>', esc_html($more_text_string) );
		}
