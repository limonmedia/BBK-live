	...
	class Woo_Variation_Swatches_Pro_Archive_Page extends Woo_Variation_Swatches_Pro_Product_Page {
		...
		public function add_to_cart_variation() {
			...
			if ( $product && 'variation' === $product->get_type() ) {
				...
				$variation    = $this->find_attributes( $data );
			}
			...
			if ( $passed_validation && false !== WC()->cart->add_to_cart( $product_id, $quantity, $variation_id, $variation ) && 'publish' === $product_status ) {
				...
				$data = array(
					'error'       => true,
					'product_url' => esc_url(  apply_filters( 'woocommerce_cart_redirect_after_error', get_permalink( $product_id ), $product_id )),
				);
		...
		public function add_to_cart_args( $args, $product ) {
			...
			if ( $product->is_type( 'variable' ) && wc_string_to_bool( woo_variation_swatches()->get_option( 'show_on_archive', 'yes' ) ) ) {
				...
				$classes = empty( $args[ 'class' ] ) ? array() : array_unique( explode( ' ', $args[ 'class' ] ) );
		...
		public function get_variations() {
			...
			$is_archive = wc_string_to_bool( $data[ 'is_archive' ] );
		...
		public function get_variation() {
			...
			$variable_product = wc_get_product( absint( $data[ 'product_id' ] ) );
			...
			if ( $enable_catalog_mode ) {
				$variation_id = $this->find_matching_product_variation( $variable_product, wp_unslash( $data ) );
			} else {
				$data_store = WC_Data_Store::load( 'product' );
				// $data_store   = $variable_product->get_data_store();
				$variation_id = $data_store->find_matching_product_variation( $variable_product, wp_unslash( $data ) );
			}
		...
		public function extra_js_options( $options ) {
			...
			$options[ 'disable_out_of_stock' ]                     = wc_string_to_bool( woo_variation_swatches()->get_option( 'hide_out_of_stock_variation', 'yes' ) );
			$options[ 'deselect_unavailable' ]                     = wc_string_to_bool( woo_variation_swatches()->get_option( 'deselect_unavailable', 'no' ) );
			$options[ 'clickable_out_of_stock' ]                   = wc_string_to_bool( woo_variation_swatches()->get_option( 'clickable_out_of_stock_variation', 'no' ) );
			$options[ 'unavailable_tooltip_text' ]                 = esc_html__( '(Unavailable)', 'woo-variation-swatches-pro' );
			$options[ 'out_of_stock_tooltip_text' ]                = esc_html__( '(Out of Stock)', 'woo-variation-swatches-pro' );
			...
			$options[ 'enable_ajax_add_to_cart' ]                  = sanitize_text_field(get_option( 'woocommerce_enable_ajax_add_to_cart', 'yes' ));
		...
		public function get_available_preview_variation( $variation, $product ) {
			...
			$available_variation = wp_cache_get( $cache_key, $cache_group );

			if ( false === $available_variation ) {
		...
		public function get_available_variations( $product ) {
			...
			$available_variations = wp_cache_get( $cache_key, $cache_group );
		...
		public function archive_product_dropdown( $html, $args ) {
			...
			if ( $convert_to_image && 'select' === $attribute_type ) {
				$attributes = $product->get_variation_attributes();
				// $attributes      = $this->get_cached_variation_attributes( $product );
			...
			if ( $default_to_button && 'select' === $attribute_type ) {
				$attribute_type = 'button';
			}

			if ( in_array( $attribute_type, array( 'mixed', 'custom', 'color', 'radio', 'image', 'button' ), true ) ) {
			...
			if ( ! empty( $options ) ) {
				if ( $product && taxonomy_exists( $attribute ) ) {
					...
					foreach ( $options as $option_index=>$option ) {
						...
						$swatches_data[] = $this->get_swatch_data( $args, $option, $option_index );
			...
			if ( ! empty( $options ) && ! empty( $swatches_data ) && $product ) {
				...
				foreach ( $swatches_data as $data ) {
					...
					if ( ! $enable_catalog_mode && 0 < $global_archive_display_limit && ( $increment + 1 ) === $global_archive_display_limit  ) {
						$incremented = $increment;
					}

					// Catalog mode and Display limit with navigate behaviour.
					if ( $enable_catalog_mode && 0 < $catalog_mode_display_limit && ( $increment + 1 ) === $catalog_mode_display_limit  ) {
						$incremented = $increment;
